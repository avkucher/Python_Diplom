...
class SignUpTestCase(APITestCase):
    ...
    def test_password_does_not_match(self):
        response = self.client.post(
            reverse('signup'),
            {
                'username': 'test_user',
                'password': 'a1s2d3f$',
                'password_repeat': '1a2s3d$f',
            }
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertDictEqual(
            response.json(),
            {'non_field_errors': ['Password must match.']}
        )
    ...
    def test_minimal_required_fields_success(self):
        ...
        self.assertNotEqual(new_user.password, 'a1s2d3f$')
        self.assertTrue(new_user.check_password('a1s2d3f$'))
